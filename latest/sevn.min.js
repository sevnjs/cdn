function SevnJS(){const license="copyrights Prateek Raj Gautam, soon to be released under Apache 2.0";const version=`v0.8.3`;const grab=(parentidstr)=>{var match=parentidstr.match(/^([^\[]*)\[*(\d*)\]*$/mi);var parentid=match[1];var no=match[2]>0?match[2]:0;var parentElement=null;try{if(parentid instanceof Object==true){parentElement=parentid}
...
const tablerender=(table)=>{var tableHeadBodySepratorPattern=/(^\|)(?=(:|-))([^\w\d\s]*?)(\|\s*$)/gm;var sep=table.matchAll(tableHeadBodySepratorPattern);var sep=Array.from(sep);if(sep.length>0){var Sep=sep[0][0];var alignment=Sep.substr(1,Sep.length-2);alignment=alignment.replaceAll(":---:","center").replaceAll(":---","left").replaceAll("---:","right").replaceAll("---","justify");alignment=alignment.split("|");var T=table.split(Sep);var thead=T[0];var tbody=T[1];thead=`\n\t<thead>${tableRowsParser(thead,alignment)}\n</thead>`;tbody=`\n\t<tbody>${tableRowsParser(tbody,alignment)}\n</tbody>`;}
...
var R=RowParser(Row[3],alignment);table=table.replaceAll(Row[0],R);});return table};function RowParser(Row,alignment){var parsedRow=""
var Col=Row.split("|")
...
const textformat=(md)=>{var inlinecodePattern=/(?<!`)`([^`]*?)`(?!`)/gmi;match1=md.matchAll(inlinecodePattern);matchList=Array.from(match1);matchList.forEach(p=>{md=md.replaceAll(p[0],`<code class='parsemd-code code-inline'>${p[1]}</code>`);})
var blockMathPattern=/\${2}([^$\n]+)\${2}/gm
match1=md.matchAll(blockMathPattern)
matchList=Array.from(match1)
matchList.forEach(p=>{md=md.replaceAll(p[0],`\\[${p[1]}\\]`)})
var inlineMathPattern=/(?<!\$)\$([^$\n]+)\$(?!\$)/mg;match1=md.matchAll(inlineMathPattern)
matchList=Array.from(match1)
matchList.forEach(p=>{md=md.replaceAll(p[0],`\\(${p[1]}\\)`)})
var imageUrlPattern=/!\[([^\]]*)]\("?([^\)"']*)"?\)/gm
match1=md.matchAll(imageUrlPattern)
matchList=Array.from(match1)
matchList.forEach(p=>{md=md.replaceAll(p[0],`\n<img src="${p[2]}"alt="${p[1]}"/>`)})
var linkPattern=/(?<!!)\[([^\]]*)\]\(([^\)]*)\)/gmi
match1=md.matchAll(linkPattern)
matchList=Array.from(match1)
matchList.forEach(p=>{md=md.replaceAll(p[0],`<a href="${p[2]}">${p[1]}</a>`)})
...
matchList=Array.from(match1)
matchList.forEach(p=>{if(p[1][0]=="~"){md=md.replace(p[0],`<del class='parsedmd-del'>${p[2]}</del>`)}
else if(p[1][0]=="="){md=md.replace(p[0],`<mark class='parsedmd-mark'>${p[2]}</mark>`)}})
https:var referencelinkPattern=/(?<!!)\[([^\]]*)\]\s{0,3}\[([^\]]*)\]/gmi
...
matchList=Array.from(match1)
matchList.forEach(p=>{var block=p[0]
var listPattern=/^\[([^\]]*)\]:\s+([^\n]*)$/gmi
list=block.matchAll(listPattern)
listEntry=Array.from(list)
listEntry.forEach(li=>{block=block.replaceAll(li[0],`\n\t<span id="${li[1]}">${li[2]}</span>`)})
...
var lex=[]
while(mdinput.length>0){var res=checkBlockTypeStage1(mdinput);if(res!=null){var type=res.type;var content=res.match;var unprocessed=res.unprocessed;if(content.length>0){mdinput=unprocessed;lex.push({type:type,content:content,})}}}
...
callback(rend1join)
return rend1join;}
const append=(parentid,childhtml,position='after')=>{position=position.toLowerCase()
try{if(parentid instanceof Object==true){var parentElement=parentid}
...
T.id='T'
T.innerHTML=""
if(Array.isArray(childhtml)==true){childhtml.forEach((child,index)=>{if(typeof child=='string'){T.innerHTML+=child}
...
if(position=='before'||position=='b'){parentElement.innerHTML=T.innerHTML+parentElement.innerHTML}else if(position=='over'||position=='o'){if(T.innerHTML!=null)parentElement.innerHTML=T.innerHTML
if(T.innerHTML==null)parentElement.innerHTML=''}else if(position=='replace'||position=='r'){parentElement.outerHTML=T.innerHTML}else if(position=='after'||position=='a'){parentElement.innerHTML=parentElement.innerHTML+T.innerHTML}else if(position=='parent'||position=='p'){var oldElement=parentElement.outerHTML
parentElement.innerHTML=""
T.childNodes[0].innerHTML+=oldElement
parentElement.outerHTML=T.innerHTML}else{parentElement.innerHTML=parentElement.innerHTML+T.innerHTML}}
catch(err){console.log(`append(${parentid},${childhtml},${position})`)
console.error(err)}};const gen=(elementtype,idin,htmlin,classin,src,event)=>{try{if(htmlin!=undefined){if(Array.isArray(htmlin)!=true){var element=document.createElement(elementtype);if(idin!=undefined&&idin!=""){element.id=idin;}
...
for(var jj=0;jj<htmlin.length;jj++){if(typeof checkfirstinput!='object'){var elementarray=document.createElement(elementtype);if(idin!=undefined&&idin!=""){elementarray.id=`${idin}-${jj}`;}
...
if(typeof checkfirstinput=='object'){elementarray=jsonToElement(htmlin[jj])
...
return element;}
catch(err){console.error("Error during gen(",elementtype,idin,htmlin,classin,src,")",err)}};const load=(srcList,pos="head")=>{const loadlink=(currentLink)=>{currentLink
var extensionStart=0
var extensionEnd=currentLink.length
var extension=""
if(currentLink.length>0){for(let i=extensionEnd;i>0;i--){if(currentLink[i]=="."){extensionStart=i;extension=currentLink.slice(extensionStart,extensionEnd)
break}}
...
catch(err){console.log(srcList)
console.error(err)}}
var self={}
self=(...args)=>{console.log(args.length)
if(args.length==1){let selector=args[0];return document.querySelectorAll(selector)}
