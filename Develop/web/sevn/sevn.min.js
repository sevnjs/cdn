...
var blockMathPattern=/\${2}([^$\n]+)\${2}/gm;match1=md.matchAll(blockMathPattern);matchList=Array.from(match1);matchList.forEach(p=>{md=md.replaceAll(p[0],`\\[${p[1]}\\]`);})
var inlineMathPattern=/(?<!\$)\$([^$\n]+)\$(?!\$)/mg;match1=md.matchAll(inlineMathPattern);matchList=Array.from(match1);matchList.forEach(p=>{md=md.replaceAll(p[0],`\\(${p[1]}\\)`);})
...
match1=md.matchAll(strikethroughPattern);matchList=Array.from(match1);matchList.forEach(p=>{if(p[1][0]=="~"){md=md.replace(p[0],`<del class='parsedmd-del'>${p[2]}</del>`)}
