function SevnJS(){const license="copyrights Prateek Raj Gautam, soon to be released under Apache 2.0";const version=`v0.9.5`;const grab=(parentidstr)=>{var match=parentidstr.match(/^([^\[]*)\[*(\d*)\]*$/mi);var parentid=match[1];var parentElement=null;try{if(parentid instanceof Object==true){parentElement=parentid}
...
const splitList=(str,sep=",")=>{if(str===undefined||str===null||str==="")return[];return String(str).split(sep).map((s)=>s.trim()).filter((s)=>s.length>0);};const applyClass=(element,classStr)=>{if(!classStr)return;splitList(classStr).forEach((cls)=>element.classList.add(cls));};const setContent=(element,content)=>{if(content===undefined||content===null)return;if(content instanceof Node){element.appendChild(content);return;}
