function SevnJS(){const license="copyrights Prateek Raj Gautam, soon to be released under Apache 2.0";const version=`v0.9.4-beta`;const grab=(parentidstr)=>{var match=parentidstr.match(/^([^\[]*)\[*(\d*)\]*$/mi);var parentid=match[1];var parentElement=null;try{if(parentid instanceof Object==true){parentElement=parentid}
...
if(htmlin[jj].nodeName===undefined){if(typeof(htmlin)!="object"){elementarray.innerHTML=htmlin[jj];if(elementtype=='input')elementarray.value=htmlin[jj];if(elementtype=='img')element.alt=htmlin[jj];if(elementtype=='script'){element.textContent=htmlin[jj];};}
if(typeof(htmlin)=="object"){elementarray.innerHTML=htmlin[jj];if(elementtype=='input')elementarray.value=htmlin[jj];if(elementtype=='img')element.alt=htmlin[jj];if(elementtype=='script'){element.textContent=htmlin[jj];};}};if(htmlin[jj].nodeName!=undefined){elementarray.append(htmlin[jj]);};if(classin!=undefined&&classin!=""){elementarray.classList+=classin.replaceAll(',',' ').replaceAll(', ',' ');}}
