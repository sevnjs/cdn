function SevnJS(){const license="copyrights Prateek Raj Gautam, soon to be released under Apache 2.0";const version=`v0.9.6`;const grab=(parentidstr)=>{var match=parentidstr.match(/^([^\[]*)\[*(\d*)\]*$/mi);var parentid=match[1];var parentElement=null;try{if(parentid instanceof Object==true){parentElement=parentid}
else{parentElement=document.querySelectorAll(parentid);}}
catch(err){console.log(`grab(${parentidstr})`);console.error(err);}
if(match[2].length>0)return parentElement[match[2]];else return parentElement;};const parsemd=(mdinput,callback)=>{var scriptArray=[];var blockPattern={code:/^\s*```(?:(\w*)\n)?([\s\S]*?)```[\s\n]*/mi,html1:/^<(?!\/)([^>\s]*)[^>]*>[\s\S]*?<\/\1>[\s\n]*/mi,html2:/<(br|hr|img|area|base|col|embed|input|link|meta|param|source|track|wbr)[^\>]*?>\s*\n*/i,heading1:/^\#{1,6}\s+([^\n]*)[\s\n]*/i,heading2:/^(\w[^\n]*)\n(-|\=){4,}[\s\n]*/im,block:/^^\s*>\s+([^\n]*)[\s\n]*/i,checklist:/^\s*(?:^\s*-\s+\[(?:\s*|[xX*])\]\s+[^\n]*\n?)+/m,table:/^\s*(?:^\s*\|.*\|\s*\n?)+/m,list:/^\s*(?m:^(?:\s*(?:\*|-|\d+\.)\s+[^\n]+)(?:\n|$))+/m,reference:/^\n+\-{3,}$/im,paragraph:/^(^[^-#|>\s][^\n]*(?:\n[^-#|>\s`][^\n]*)*)/im,empty:/^[\s\n]*/m,unknown:/^\s*.*\n*$/mi};var checkSequence="code,html1,html2,heading1,heading2,block,checklist,list,reference,table,paragraph,empty,unknown".split(",");const checkBlockTypeStage1=(mdinput)=>{function matchMarkdownBlock(text){for(const type of checkSequence){const regex=blockPattern[type];const match=text.match(regex);if(match&&match.index===0){return{type,match:match[0],unprocessed:text.slice(match[0].length)};}}
return{type:"unknown",match:"",unprocessed:text};};for(var i=0;i<=checkSequence.length;i++){var type=checkSequence[i];var pattern=blockPattern[type];res=matchMarkdownBlock(mdinput);if(pattern==null||pattern==undefined){continue;}}
return res;};const renderpattern={code:/^\s*```(?:(\w*)\n)?([\s\S]*?)```[\s\n]*/mi,heading1:/^\#{1,6}\s+([^\n]*)[\s\n]*/i,heading2:/^(\w[^\n]*)\n(-|\=){4,}[\s\n]*/im,block:/^^\s*>\s+([^\n]*)[\s\n]*/i,checklist:/^\s*(?:^\s*-\s+\[(?:\s*|[xX*])\]\s+[^\n]*\n?)+/m,table:/^\s*(?:^\s*\|.*\|\s*\n?)+/m,list:/^\s*(?m:^(?:\s*(?:\*|-|\d+\.)\s+[^\n]+)(?:\n|$))+/m,reference:/^\n+\-{3,}$/im,paragraph:/^(^[^-#|>\s][^\n]*(?:\n[^-#|>\s][^\n]*)*)/im,empty:/^\s*[\s\n]*/m,};const coderender=(block)=>{var pattern=/^\s*```([^\n]*)\n([\s\S]*)```[\s\n]*/mi;const match=block.match(pattern);if(match){var matchArray=Array.from(match);var content=self.escapeHtml(matchArray[2]);var lang=matchArray[1];var res=`<pre><code class="language-${lang} ${lang} code-block">${content}</code></pre>`;return res;}}
...
const textformat=(md)=>{var inlinecodePattern=/(?<!`)`([^`]*?)`(?!`)/gmi;match1=md.matchAll(inlinecodePattern);matchList=Array.from(match1);matchList.forEach(p=>{var htmlsafecode=self.escapeHtml(p[1])
...
var italicPattern=/([*_]{1,3})(\S.*?\S)\1/g;match1=md.matchAll(italicPattern)
...
var referencelinkPattern=/(?<!!)\[([^\]]*)\]\s{0,3}\[([^\]]*)\]/gmi;match1=md.matchAll(referencelinkPattern);matchList=Array.from(match1);matchList.forEach(p=>{md=md.replaceAll(p[0],`<a href="#${p[2]}">${p[1]}</a>`);})
...
function extractscript(htmlstr){const parser=new DOMParser();const docstr=parser.parseFromString(htmlstr,"text/html");const scripts=docstr.querySelectorAll("script");const scriptstr=Array.from(scripts).map(scr=>scr.innerHTML)
return scriptstr}
for(var j=0;j<lex.length;j++){var block=lex[j]
var rend=""
if(block.type=="code"){var rend=coderender(block.content);}
else if(block.type=="html1"||block.type=="html2"){var rend=block.content;scriptArray.push(extractscript(rend))}
...
var scriptString=scriptArray.join();if(callback!=undefined&&typeof callback==="function"){callback(rend1join,scriptString);}
return rend1join,scriptString;}
const append=(parentid,childhtml,position='after')=>{position=position.toLowerCase();try{let parentElement;if(parentid instanceof Object){parentElement=parentid}else{parentElement=document.querySelectorAll(parentid)[0];}
var parentTag=parentElement.tagName.toLowerCase();if(parentTag=='table'||parentTag=='thead'||parentTag=='tbody'||parentTag=='tr'||parentTag=='td'){var T=document.createElement('table');T.id='T';T.innerHTML="";}else{var T=document.createElement('div');T.id='T';T.innerHTML="";}
if(Array.isArray(childhtml)){childhtml.forEach((child,index)=>{if(typeof child=='string'){T.innerHTML+=child;}else if(typeof child!='string'){if(child.outerHTML!=undefined){T.appendChild(child);}
...
function appendScript(parentElement,scripttag){var newScript=document.createElement('script');newScript.textContent=scripttag.textContent;newScript.id=scripttag.id||'';parentElement.appendChild(newScript);};var parser=new DOMParser();var appending=parser.parseFromString(T.innerHTML,"text/html");var scripts=appending.querySelectorAll("script");if(scripts.length==1&&scripts[0].outerHTML.length==T.innerHTML.length){try{appendScript(parentElement,scripts[0]);T.innerHTML="";}catch(error){console.error("Error appending script or not a script:",error);}};if(position=='before'||position=='b'){parentElement.innerHTML=T.innerHTML+parentElement.innerHTML;}else if(position=='over'||position=='o'){if(T.innerHTML!=null)parentElement.innerHTML=T.innerHTML;if(T.innerHTML==null)parentElement.innerHTML='';}else if(position=='replace'||position=='r'){parentElement.outerHTML=T.innerHTML;}else if(position=='after'||position=='a'){parentElement.innerHTML=parentElement.innerHTML+T.innerHTML;}else if(position=='parent'||position=='p'){var oldElement=parentElement.outerHTML;parentElement.innerHTML="";T.childNodes[0].innerHTML+=oldElement;parentElement.outerHTML=T.innerHTML;}else{parentElement.innerHTML=parentElement.innerHTML+T.innerHTML;}}catch(err){console.log(`append(${parentid},${childhtml},${position})`);console.error(err);}};const gen=(elementtype,idin,htmlin,classin,src)=>{if(idin!==undefined&&idin!==null&&typeof idin!=="string"){src=classin;classin=htmlin;htmlin=idin;idin="";}
const splitList=(str)=>{if(str===undefined||str===null||str==="")return[];return String(str).split(/[,; ]+/).map((s)=>s.trim()).filter((s)=>s.length>0);};const applyClass=(element,classStr)=>{if(!classStr)return;splitList(classStr).forEach((cls)=>element.classList.add(cls));};const setContent=(element,content)=>{if(content===undefined||content===null)return;if(content instanceof Node){element.appendChild(content);return;}
if(typeof content==="object"){for(const[key,value]of Object.entries(content)){if(key==="innerHtml"||key==="innerHTML"){setContent(element,value);}else if(key==="id"){element.id=value;}else if(key==="class"||key==="className"){applyClass(element,value);}else{element.setAttribute(key,value);}}
return;}
const text=String(content);const tag=element.tagName.toLowerCase();if(tag==="input"||tag==="textarea"||tag==="select"){element.value=text;}else if(tag==="img"){element.alt=text;}else if(text.includes("<")&&text.includes(">")){element.innerHTML=text;}else{element.textContent=text;}};const applySrc=(element,srcVal)=>{if(srcVal===undefined||srcVal===null)return;if(typeof srcVal==="object"){for(const[key,value]of Object.entries(srcVal)){element.setAttribute(key,value);}}else if(element.tagName.toLowerCase()==="a"||element.tagName.toLowerCase()==="link"){element.href=srcVal;}else{element.src=srcVal;}};const buildChainItem=(tagChain,idChain,content,classForItem)=>{let outerEl=null;let innerEl=null;tagChain.forEach((tag,i)=>{const el=document.createElement(tag);if(idChain[i])el.id=idChain[i];if(innerEl)innerEl.appendChild(el);else outerEl=el;innerEl=el;});setContent(innerEl,content);if(classForItem)applyClass(outerEl,classForItem);return outerEl;};const buildContentHolder=(tagSeg,idSeg,content,classForHolder)=>{const tagChain=splitList(tagSeg,"+");const idChainRaw=splitList(idSeg,"+");if(!Array.isArray(content)){return buildChainItem(tagChain,idChainRaw,content,classForHolder);}
return content.map((item,index)=>{const idChain=idChainRaw.map((id)=>`${id}-${index}`);return buildChainItem(tagChain,idChain,item,classForHolder);});};const wrapItems=(tag,id,items)=>{const wrapper=document.createElement(tag);if(id)wrapper.id=id;items.forEach((item)=>wrapper.appendChild(item));return wrapper;};try{const tagLevels=splitList(elementtype);const idLevels=splitList(idin);if(tagLevels.length===0)throw new Error("elementtype must have at least one tag");const holderTagSeg=tagLevels[tagLevels.length-1];const holderIdSeg=idLevels[tagLevels.length-1];let result=buildContentHolder(holderTagSeg,holderIdSeg,htmlin,classin);if(tagLevels.length===1){if(Array.isArray(result)){result=wrapItems(holderTagSeg.split("+")[0],"",result);}}else{for(let i=tagLevels.length-2;i>=0;i--){if(Array.isArray(result)){result=wrapItems(tagLevels[i],idLevels[i],result);}else{const wrapper=document.createElement(tagLevels[i]);if(idLevels[i])wrapper.id=idLevels[i];wrapper.appendChild(result);result=wrapper;}}}
applySrc(result,src);return result;}catch(err){console.error("Error during gen(",elementtype,idin,htmlin,classin,src,")",err);return null;}};const gens=(...args)=>{const el=gen(...args);return el&&el.outerHTML?el.outerHTML.toString():null;};const genp=(...args)=>{const html=gens(...args);if(html===null)return null;return html.replaceAll("&","&amp;").replaceAll("</","&lt;&#47;").replaceAll("<","&lt;").replaceAll(">","&gt;");};const load=(srcList,pos="head")=>{const loadlink=(currentLink)=>{var extensionStart=0;var extensionEnd=currentLink.length;var extension="";if(currentLink.length>0){for(let i=extensionEnd;i>0;i--){if(currentLink[i]=="."){extensionStart=i;extension=currentLink.slice(extensionStart,extensionEnd);break}}
